...
int main(){
  ...
  for(int it = 0 ; it <= IT_MAX ; it ++){  
    ...
    fprintf(file, "%f %f %f\n", it*delta, c, x_sr);
    if (it == 0){
      write_to_file(U, "U0_b.dat");
    }
    if(it == 300){
      write_to_file(U, "U1_b.dat");
    }
    if(it == 600){
      write_to_file(U, "U2_b.dat");
    }
    if(it == 900){
      write_to_file(U, "U3_b.dat");
    }
    if(it == 1200){
      write_to_file(U, "U4_b.dat");
    }
    if(it == 1500){
      write_to_file(U, "U5_b.dat");
    }
    if(it%100 == 0){
      printf("%d %f\n", it, c);
    }
